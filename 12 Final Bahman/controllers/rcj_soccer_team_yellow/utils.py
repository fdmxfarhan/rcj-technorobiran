...
def readData(robot):
    ...
    if time.time() - robot.start_time > step:
        if dist(robot.last_xb, robot.last_yb, robot.xb, robot.yb) < threshold:
            ...
            if robot.xb - robot.last_xb != 0 and robot.yb - robot.last_yb != 0:
                m = (robot.yb - robot.last_yb)/(robot.xb - robot.last_xb)
                b = robot.yb - m * robot.xb
                if    robot.yb > 0.4:   robot.static_yb = 0.4
                elif  robot.yb > 0:     robot.static_yb = 0
                elif  robot.yb > -0.4:  robot.static_yb = -0.4
                else:                   robot.static_yb = robot.yb
                robot.static_xb = (robot.static_yb - b)/m
...
def Formation(robot):
    ...
    if robot.robot.getName()[1] == '3':
        moveAndLook(robot, 0.3, -0.2, 0, 0.75)
def LackOffProgress(robot):
    spot = get_nearest_neutral_spot(robot)
    if (robot.player_id == 2 and robot.ball_distance > robot.robotposes[2][2]) or (robot.player_id == 3 and robot.ball_distance > robot.robotposes[1][2]):
        moveAndLook(robot, spot[2], spot[3], 0, 0.75)